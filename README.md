...
We use a sessionId stored in localStorage.


It stays the same unless the user clears localStorage or switches browser/device.


Why not fingerprinting? Compared to fingerprinting, sessionId is safer, more privacy-friendly, easier to explain to users, and simpler to implement, also with low cost. 


It links together all events (enter, click, scroll, leave) into one session, with low performance overhead — a good fit for this project.

In details:

The sessionID is stored in localStorage. As long as localStorage is not cleared, the sessionID remains unchanged unless localStorage is cleared or the browser/device is changed.

The biggest advantages of using this sessionID (a random value generated by localStorage) over the fingerprint are security and improved compliance. Furthermore, the sessionID effectively links all user events (enter, click, leave, etc.) (Here, We are reading the data for the most recent time a user logged in and logged out of the website). We researched the advantages of using session IDs online. They offer cleaner data, are less time-consuming, and simple to implement. Session IDs are easier for users to understand as "this is an identifier for a browser session," rather than "device-level tracking" like fingerprinting. Since our project is relatively small, we believe session IDs are more suitable for this task.



2. Metric Selection
   
We tested different metrics (screen size, language, etc.). Some, like screen size, were not meaningful for our analysis.


We chose user language because it reflects the audience and is valuable for:


Developers (decide which features, localized products to build)


Owners (know where their audience is, better targeting by spending the least)


Users ( let user having better experience, such display in their preferred languages, build more trust through tech or emotionally support)


3. Chart Choices
   
Pie Chart (screen size): included to illustrate variety, but not highly insightful.


Pie Chart (language): useful for presentations but messy with more than 4+ languages.


Bar Chart (language): clearer comparisons across categories; chosen as the main analysis view.


Tables (grid):


Latest user entered / left time
One detailed table (latest 50 user records: id, sessionId, language, timestamp)


4. Dashboard Metrics （link back to the guiding question:)
   
On reporting.cse135.online, we provide three key views:

Activity Counts → shows user engagement over time.


User Language Preferences → detailed in language.html.


Latest Session Records → originally showed one user; improved to display the latest 50 users for better insight into session behavior.
User-centered framing: basically from this class the take away is learned how to balance as a developer working presenting and between owner, user benefits, all the decisions were made for usability, clarity, and insight.


Reflection: I wish we had more time to make this project better, but we are still satisfied with what we learned. Thank you. 
